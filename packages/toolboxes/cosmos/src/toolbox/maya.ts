...
const broadcastMultisigTx = async (
  ...
  const addressesAndSignatures: [string, Uint8Array][] = signers.map((signer) => [
    pubkeyToAddress(
      ...
      isStagenet ? 'smaya' : 'maya',
    ),
    base64.decode(signer.signature),
  ]);

  const broadcaster = await StargateClient.connect(isStagenet ? RPCUrl.MayaStagenet : RPCUrl.Maya);
...
const __REEXPORT__pubkeyToAddress = async (pubkey: Pubkey, prefix = 'maya') => {
...
export const MayaToolbox = ({ stagenet }: ToolboxParams): MayaToolboxType => {
  const rpcUrl = getRPC(ChainId.Maya, stagenet);
  const nodeUrl = stagenet ? ApiUrl.MayanodeStagenet : ApiUrl.MayanodeMainnet;

  const client = new CosmosClient({
    server: nodeUrl,
    chainId: ChainId.Maya,
    prefix: stagenet ? 'smaya' : 'maya',
    stagenet,
  });
  ...
  const baseToolbox: {
    ...
    client,
    derivationPath: DerivationPath.MAYA,
  ...
  const getFees = async () => {
    ...
    try {
      ...
      } = await getRequest<ThorchainConstantsResponse>(`${nodeUrl}/mayachain/constants`);
      ...
      fee = new SwapKitNumber({ value: 1, decimal: BaseDecimal.MAYA });
    }
  ...
  const deposit = async ({ signer, assetValue, memo, from }: DepositParam & { from: string }) => {
    ...
    const assetObj = assetValue.isSynthetic
      ? {
